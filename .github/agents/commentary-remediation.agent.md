...
3. **Validation loop (no shortcuts):** Run the full toolchain in order after edits, repeating until clean: `poetry run black .`, `poetry run ruff check`, `poetry run pyright`, `poetry run pytest --cov=invoice_etl --cov-report=term-missing`. If any step changes files or fails, fix issues and restart from Black.
